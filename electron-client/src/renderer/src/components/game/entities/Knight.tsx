...
Command: npx gltfjsx@6.2.15 public/models/knight.glb -t -s -r public/ -o ./src/components/character/Knight.tsx 
*/
...
import { useEntityAction, useEntityShadow } from "@comp/game/entities"
...
export const Model: FC<Props> = ({ action, asShadow, ...props }) => {
  ...
  const { scene, materials, animations } = useGLTF("http://localhost:4792/models/knight.glb") as GLTFResult
  const clone = useMemo(() => SkeletonUtils.clone(scene), [scene])
  const graph = useGraph(clone)
  const nodes = graph.nodes as GLTFResult["nodes"]
  const { actions } = useAnimations(animations, group)

  useEntityAction(actions, action)
